...
- Cella is now available in Spanish across both the web and mobile interfaces.
- Gate entry and appointment forms now capture a "Supplier" field for incoming goods, replacing the previous seal number/weight fields with a single "Container number" field.
- The appointment form now supports selecting carrier special requirements (e.g. customs, dangerous goods, returnable packaging), which also appear on the appointment detail view.
- The truck schedule and visitor schedule calendars now let you click the date label to jump directly to a specific day or week via a date picker.
- Locations can now be configured with an "Automatic assignment" option, visible and editable from the location extras screen.
- Box line details now display a status history section.
- A new permission-based option allows blocking manual entry of appointment names, so the appointment number is always auto-generated by the system.

### Improvements

- When assigning loads, deliveries, orders, or purchase orders to an appointment, entities already linked to that appointment are automatically excluded from the selection list, and the system now prevents accidentally assigning the same entity twice.
- The plate search on the gate entry screen can now be disabled via configuration, restricting driver lookup to QR code or appointment number only.
- Gate entry driver plate lookup is now case-insensitive and only matches confirmed appointments, avoiding accidental matches with past or cancelled appointments.
- Visitor registration no longer requires at least one of email or phone number to be filled in.
- The gate entry header no longer displays the dock/location name during check-in steps.
- Cancelled appointments are now excluded from the truck schedule view instead of appointments still "In Creation".
- The truck schedule now stays on the previously selected date when navigating instead of resetting to today.
- The supplier field is automatically hidden and cleared for outbound appointments, since it only applies to incoming goods.
- Creating a default load for an appointment now also sets its expected departure date, and the system no longer proposes creating a default load for inbound appointments when appointment/unload linking is disabled.

### Fixes

- Fixed an issue where translated labels (pallet types, special requirements) could fail to display correctly for certain language codes.
- Fixed the gate validation detail screen to correctly hide the supplier field for outbound appointments.
